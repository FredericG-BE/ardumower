...
void RemoteControl::setRobot(Robot *aRobot, Esp8266* aComms){
  this->robot = aRobot;
  comms = aComms;
}

void RemoteControl::initSerial(int baudrate){
  //comms->begin(baudrate);   // pfod app
}
...
void RemoteControl::sendYesNo(int value){
  if (value == 1) comms->print("YES");
    else comms->print("NO");
}

void RemoteControl::sendOnOff(int value){
   if (value == 1) comms->print("ON");
    else comms->print("OFF");
}

void RemoteControl::sendTimer(ttimer_t timer){
  if (timer.active) comms->print(F("(X)  "));
    else comms->print(F("(   )  "));
  comms->print(time2str(timer.startTime));
  comms->print("-");
  comms->print(time2str(timer.stopTime));
  comms->println();
  if (timer.daysOfWeek == B1111111){
    comms->print(F("every day"));
  } else {
    int counter= 0;  
    for (int j=0; j < 7; j++){
      if ((timer.daysOfWeek >> j) & 1) {
        if (counter != 0) comms->print(",");
        comms->print(dayOfWeek[j]);
        counter++;      
      }
    }
  }
...
void RemoteControl::sendSlider(String cmd, String title, float value, String unit, double scale, float maxvalue, float minvalue){
  ...
  comms->print(title);
  comms->print(" `");
  comms->print(((int)(value/scale)));
  comms->print("`");
  comms->print(((int)(maxvalue/scale)));
  comms->print("`");
  comms->print(((int)(minvalue/scale)));
  comms->print("~ ~");
  if (scale == 10) comms->print("10");
    else if (scale == 1) comms->print("1");
    else if (scale == 0.1) comms->print("0.1");
    else if (scale == 0.01) comms->print("0.01");
    else if (scale == 0.001) comms->print("0.001");
    else if (scale == 0.0001) comms->print("0.0001");
}
...
void RemoteControl::sendMainMenu(boolean update){
  if (update) comms->print("{:"); else {
    comms->print(F("{.Ardumower"));
    comms->print(" (");
    comms->print(robot->name);
    comms->print(")");
  }
  comms->print(F("|r~Commands|n~Manual|s~Settings|in~Info|c~Test compass|m1~Log sensors|yp~Plot"));
  comms->println(F("|y4~Error counters|y9~ADC calibration}"));
}

void RemoteControl::sendADCMenu(boolean update){
  if (update) comms->print("{:"); else comms->print(F("{.ADC calibration`1000"));
  comms->print(F("|c1~Calibrate (perimeter sender, charger must be off) "));
...
void RemoteControl::sendPlotMenu(boolean update){
  if (update) comms->print("{:"); else comms->print(F("{.Plot"));
  comms->print(F("|y7~Sensors|y5~Sensor counters|y3~IMU|y6~Perimeter|y8~GPS"));
  comms->println(F("|y1~Battery|y2~Odometry2D|y11~Motor control|y10~GPS2D}"));
}  


void RemoteControl::sendSettingsMenu(boolean update){
  if (update) comms->print("{:"); else comms->print(F("{.Settings"));
  comms->print(F("|sz~Save settings|s1~Motor|s2~Mow|s3~Bumper|s4~Sonar|s5~Perimeter|s6~Lawn sensor|s7~IMU|s8~R/C"));
  comms->println(F("|s9~Battery|s10~Station|s11~Odometry|s13~Rain|s15~Drop sensor|s14~GPS|i~Timer|s12~Date/time|sx~Factory settings}"));
}  

void RemoteControl::sendErrorMenu(boolean update){
  if (update) comms->print("{:"); else comms->print(F("{.Error counters`1000"));
  comms->print(F("|z00~Reset counters"));
  comms->print(F("|zz~ADC calibration "));
  comms->print(robot->errorCounterMax[ERR_ADC_CALIB]);
  comms->print(F("|zz~Charger "));
  comms->print(robot->errorCounterMax[ERR_CHARGER]);
  comms->print(F("|zz~Battery "));
  comms->print(robot->errorCounterMax[ERR_BATTERY]);
  comms->print(F("|zz~Motor left "));
  comms->print(robot->errorCounterMax[ERR_MOTOR_LEFT]);
  comms->print(F("|zz~Motor right "));
  comms->print(robot->errorCounterMax[ERR_MOTOR_RIGHT]);
  comms->print(F("|zz~Motor mow "));
  comms->print(robot->errorCounterMax[ERR_MOTOR_MOW]);
  comms->print(F("|zz~Mow sense "));
  comms->print(robot->errorCounterMax[ERR_MOW_SENSE]);
  comms->print(F("|zz~Odometry left "));
  comms->print(robot->errorCounterMax[ERR_ODOMETRY_LEFT]);
  comms->print(F("|zz~Odometry right "));
  comms->print(robot->errorCounterMax[ERR_ODOMETRY_RIGHT]);
  comms->print(F("|zz~Perimeter timeout "));
  comms->print(robot->errorCounterMax[ERR_PERIMETER_TIMEOUT]);
  comms->print(F("|zz~Perimeter tracking "));
  comms->print(robot->errorCounterMax[ERR_TRACKING]);
  comms->print(F("|zz~IMU comm "));
  comms->print(robot->errorCounterMax[ERR_IMU_COMM]);
  comms->print(F("|zz~IMU calibration "));
  comms->print(robot->errorCounterMax[ERR_IMU_CALIB]);
  comms->print(F("|zz~IMU tilt "));
  comms->print(robot->errorCounterMax[ERR_IMU_TILT]);
  comms->print(F("|zz~RTC comm "));
  comms->print(robot->errorCounterMax[ERR_RTC_COMM]);
  comms->print(F("|zz~RTC data "));
  comms->print(robot->errorCounterMax[ERR_RTC_DATA]);
  comms->print(F("|zz~GPS comm "));
  comms->print(robot->errorCounterMax[ERR_GPS_COMM]);
  comms->print(F("|zz~GPS data "));
  comms->print(robot->errorCounterMax[ERR_GPS_DATA]);
  comms->print(F("|zz~EEPROM data "));
  comms->print(robot->errorCounterMax[ERR_EEPROM_DATA]);
  comms->println("}");
}  
...
void RemoteControl::sendMotorMenu(boolean update){  
  if (update) comms->print("{:"); else comms->print(F("{.Motor`1000"));
  comms->println(F("|a00~Overload Counter l, r "));
  comms->print(robot->motorLeftSenseCounter);
  comms->print(", ");
  comms->print(robot->motorRightSenseCounter);
  comms->println(F("|a01~Power in Watt l, r "));
  comms->print(robot->motorLeftSense);
  comms->print(", ");
  comms->print(robot->motorRightSense);
  comms->println(F("|a05~motor current in mA l, r "));
  comms->print(robot->motorLeftSenseCurrent);
  comms->print(", ");
  comms->print(robot->motorRightSenseCurrent);
  ...
  comms->print(F("|a05~Speed l, r"));
  comms->print(robot->motorLeftPWMCurr);
  comms->print(", ");
  comms->print(robot->motorRightPWMCurr);
  ...
  comms->println(F("|a10~Testing is"));
  switch (testmode){
    case 0: comms->print(F("OFF")); break;
    case 1: comms->print(F("Left motor forw")); break;
    case 2: comms->print(F("Right motor forw")); break;
  }
  comms->print(F("|a14~for config file:"));
  comms->print(F("motorSenseScale l, r"));
  comms->print(robot->motorSenseLeftScale);
  comms->print(", ");
  comms->print(robot->motorSenseRightScale);
  comms->print(F("|a16~Swap left direction "));
  sendYesNo(robot->motorLeftSwapDir);
  comms->print(F("|a17~Swap right direction "));
...
void RemoteControl::sendMowMenu(boolean update){
  if (update) comms->print("{:"); else comms->print(F("{.Mow`1000"));
  comms->print(F("|o00~Overload Counter "));
  comms->print(robot->motorMowSenseCounter);
  comms->print(F("|o01~Power in Watt "));
  comms->print(robot->motorMowSense);
  comms->print(F("|o11~current in mA "));
  comms->print(robot->motorMowSenseCurrent);
  ...
  comms->print(F("|o04~Speed "));
  comms->print(robot->motorMowPWMCurr);
  sendSlider("o05", F("Speed max"), robot->motorMowSpeedMaxPwm, "", 1, 255);       
  comms->print(F("|o06~Modulate "));
  sendYesNo(robot->motorMowModulate);      
  comms->print(F("|o07~RPM "));
  comms->print(robot->motorMowRpmCurr);
  sendSlider("o08", F("RPM set"), robot->motorMowRPMSet, "", 1, 4500);     
  sendPIDSlider("o09", "RPM", robot->motorMowPID, 0.01, 1.0);      
  comms->println(F("|o10~Testing is"));
  switch (testmode){
    case 0: comms->print(F("OFF")); break;
    case 1: comms->print(F("Motor ON")); break;
  }
  comms->println(F("|o04~for config file:"));
  comms->println(F("motorMowSenseScale:"));
  comms->print(robot->motorMowSenseScale);
  comms->println("}");
}
...
void RemoteControl::sendBumperMenu(boolean update){
  if (update) comms->print("{:"); else comms->print(F("{.Bumper`1000"));
  comms->print(F("|b00~Use "));
  sendYesNo(robot->bumperUse);    
  comms->println(F("|b01~Counter l, r "));
  comms->print(robot->bumperLeftCounter);
  comms->print(", ");
  comms->print(robot->bumperRightCounter);
  comms->println(F("|b02~Value l, r "));
  comms->print(robot->bumperLeft);
  comms->print(", ");
  comms->print(robot->bumperRight);
  comms->println("}");
}

void RemoteControl::sendDropMenu(boolean update){
  if (update) comms->print("{:"); else comms->print(F("{.Drop`1000"));
  comms->print(F("|u00~Use "));
  sendYesNo(robot->dropUse);    
  comms->println(F("|u01~Counter l, r "));
  comms->print(robot->dropLeftCounter);
  comms->print(", ");
  comms->print(robot->dropRightCounter);
  comms->println(F("|u02~Value l, r "));
  comms->print(robot->dropLeft);
  comms->print(", ");
  comms->print(robot->dropRight);
  comms->println("}");
}
...
void RemoteControl::sendSonarMenu(boolean update){
  if (update) comms->print("{:"); else comms->print(F("{.Sonar`1000"));
  comms->print(F("|d00~Use "));
  sendYesNo(robot->sonarUse);
  comms->print(F("|d04~Use left "));
  sendYesNo(robot->sonarLeftUse);
  comms->print(F("|d05~Use center "));
  sendYesNo(robot->sonarCenterUse);
  comms->print(F("|d06~Use right "));
  sendYesNo(robot->sonarRightUse);
  comms->print(F("|d01~Counter "));
  comms->print(robot->sonarDistCounter);
  comms->println(F("|d02~Value l, c, r"));
  comms->print(robot->sonarDistLeft);
  comms->print(", ");
  comms->print(robot->sonarDistCenter);
  comms->print(", ");
  comms->print(robot->sonarDistRight);
...
void RemoteControl::sendPerimeterMenu(boolean update){
  if (update) comms->print("{:"); else comms->print(F("{.Perimeter`1000"));
  comms->print(F("|e00~Use "));
  sendYesNo(robot->perimeterUse);  
  comms->println(F("|e02~Value"));
  comms->print(robot->perimeterMag);
  if (robot->perimeterMag < 0) comms->print(" (inside)");
    else comms->print(" (outside)");
  ...
  comms->print(F("|e09~Use differential signal "));
  sendYesNo(robot->perimeter.useDifferentialPerimeterSignal);    
  comms->print(F("|e10~Swap coil polarity "));
  sendYesNo(robot->perimeter.swapCoilPolarity);
  comms->print(F("|e13~Block inner wheel  "));
...
void RemoteControl::sendLawnSensorMenu(boolean update){
  if (update) comms->print("{:"); else comms->print(F("{.Lawn sensor`1000"));
  comms->print(F("|f00~Use "));
  sendYesNo(robot->lawnSensorUse);  
  comms->print(F("|f01~Counter "));
  comms->print(robot->lawnSensorCounter);
  comms->println(F("|f02~Value f, b"));
  comms->print(robot->lawnSensorFront);
  comms->print(", ");
  comms->print(robot->lawnSensorBack);
  comms->println("}");
}
...
void RemoteControl::sendRainMenu(boolean update){
  if (update) comms->print("{:"); else comms->print(F("{.Rain`1000"));
  comms->print(F("|m00~Use "));
  sendYesNo(robot->rainUse);  
  comms->print(F("|m01~Counter "));
  comms->print(robot->rainCounter);
  comms->println(F("|m02~Value"));
  comms->print(robot->rain);
  comms->println("}");
}
...
void RemoteControl::sendGPSMenu(boolean update){
  if (update) comms->print("{:"); else comms->print(F("{.GPS`1000"));
  comms->print(F("|q00~Use "));
...
void RemoteControl::sendImuMenu(boolean update){
  if (update) comms->print("{:"); else comms->print(F("{.IMU`1000"));
  comms->print(F("|g00~Use "));
  sendYesNo(robot->imuUse);
  comms->print(F("|g01~Yaw "));
  comms->print(robot->imu.ypr.yaw/PI*180);
  comms->print(F(" deg"));
  comms->print(F("|g09~DriveHeading "));
  comms->print(robot->imuDriveHeading/PI*180);
  comms->print(F(" deg"));
  comms->print(F("|g02~Pitch "));
  comms->print(robot->imu.ypr.pitch/PI*180);
  comms->print(F(" deg"));
  comms->print(F("|g03~Roll "));
  comms->print(robot->imu.ypr.roll/PI*180);
  comms->print(F(" deg"));
  comms->print(F("|g04~Correct dir "));
  ...
  comms->print(F("|g07~Acc cal next side"));
  comms->print(F("|g08~Com cal start/stop"));
  comms->println("}");
}
...
void RemoteControl::sendRemoteMenu(boolean update){
  if (update) comms->print("{:"); else comms->print(F("{.Remote R/C`1000"));
  comms->print(F("|h00~Use "));
...
void RemoteControl::sendBatteryMenu(boolean update){
  if (update) comms->print("{:"); else comms->print(F("{.Battery`1000"));
  comms->print(F("|j00~Battery "));
  comms->print(robot->batVoltage);
  comms->print(" V");
  comms->print(F("|j01~Monitor "));
  ...
  comms->print(F("|j04~Charge "));
  comms->print(robot->chgVoltage);
  comms->print("V ");
  comms->print(robot->chgCurrent);
  comms->print("A");
...
void RemoteControl::sendStationMenu(boolean update){
  if (update) comms->print("{:"); else comms->print(F("{.Station`1000"));
...
void RemoteControl::sendOdometryMenu(boolean update){
  if (update) comms->print("{:"); else comms->print(F("{.Odometry2D`1000"));
  comms->print(F("|l00~Use "));
  sendYesNo(robot->odometryUse);  
  comms->print(F("|l01~Value l, r "));
  comms->print(robot->odometryLeft);
  comms->print(", ");
  comms->println(robot->odometryRight);
  comms->println(F("|l03~RPM Motor l, r "));
  comms->print(robot->motorLeftRpmCurr);
  comms->print(", ");
  comms->println(robot->motorRightRpmCurr);
  ...
  comms->print(F("|l08~Use two-way encoder "));
  sendYesNo(robot->twoWayOdometrySensorUse);
  comms->print(F("|l05~Swap left direction "));
  sendYesNo(robot->odometryLeftSwapDir);
  comms->print(F("|l06~Swap right direction "));
...
void RemoteControl::sendDateTimeMenu(boolean update){
  if (update) comms->print("{:"); else comms->print(F("{.Date/time"));
  comms->print("|t00~");
  comms->print(date2str(robot->datetime.date));
  comms->print(", ");
  comms->print(time2str(robot->datetime.time));
...
void RemoteControl::sendTimerDetailMenu(int timerIdx, boolean update){
  if (update) comms->print("{:"); else comms->print(F("{.Details"));
  comms->print("|p0");
  comms->print(timerIdx);
  comms->print("~Use ");
  ...
  for (int i=0; i < 7; i++){
    ...
    comms->print(i);
    comms->print("~");
    if ((robot->timer[timerIdx].daysOfWeek >> i) & 1) comms->print("(X)  ");
      else comms->print("(  )  ");
    comms->print(dayOfWeek[i]);
  }
  comms->print("|p9");
  comms->print(timerIdx);
  comms->print(F("~Set to current time"));
  comms->println("}");
}
...
void RemoteControl::sendTimerMenu(boolean update){
  if (update) comms->print("{:"); else comms->print(F("{.Timer"));
  comms->print(F("|i99~Use "));
...
void RemoteControl::sendFactorySettingsMenu(boolean update){
  if (update) comms->print("{:"); else comms->println(F("{.Factory settings"));
  comms->print(F("|x0~Set factory settings (requires reboot)"));
  comms->println("}");
}
...
void RemoteControl::sendInfoMenu(boolean update){
  if (update) comms->print("{:"); else comms->print(F("{.Info"));
  comms->print(F("|v00~Ardumower "));
  comms->print(VER);
  comms->print(F("|v01~Developer "));
  sendYesNo(robot->developerActive);        
  //comms->print("|d01~Perimeter v");
  //comms->print(verToString(readPerimeterVer()));
  //comms->print("|d02~IMU v");
  //comms->print(verToString(readIMUver()));
  //comms->print("|d02~Stepper v");
  //comms->print(verToString(readStepperVer()));
  comms->println("}");
}
...
void RemoteControl::sendCommandMenu(boolean update){  
  if (update) comms->print("{:"); else comms->print(F("{.Commands`5000"));
  comms->print(F("|ro~OFF|ra~Auto mode|rc~RC mode|"));
  comms->print(F("rm~Mowing is "));
  sendOnOff(robot->motorMowEnable);  
  comms->print(F("|rp~Pattern is "));
  comms->print(robot->mowPatternName());
  comms->print(F("|rh~Home|rk~Track|rs~State is "));
  comms->print(robot->stateName());
  comms->print(F("|rr~Auto rotate is "));
  comms->print(robot->motorLeftPWMCurr);
  comms->print(F("|r1~User switch 1 is "));
  sendOnOff(robot->userSwitch1);  
  comms->print(F("|r2~User switch 2 is "));
  sendOnOff(robot->userSwitch2);  
  comms->print(F("|r3~User switch 3 is "));
...
void RemoteControl::sendManualMenu(boolean update){
  if (update) comms->print("{:"); else comms->println(F("{^Manual navigation`1000"));
  comms->print(F("|nl~Left|nr~Right|nf~Forward"));
  if (   ((robot->motorLeftSpeedRpmSet  < 5)  && (robot->motorLeftSpeedRpmSet  > -5))
     &&  ((robot->motorRightSpeedRpmSet < 5)  && (robot->motorRightSpeedRpmSet > -5))  ){
    comms->print(F("|nb~Reverse"));
  } else comms->print(F("|ns~Stop"));
  comms->print(F("|nm~Mow is "));
  sendOnOff(robot->motorMowEnable);  
  comms->println("}");
}

void RemoteControl::sendCompassMenu(boolean update){
  if (update) comms->print("{:"); else comms->println(F("{^Compass`1000"));
  comms->print(F("|cw~West|ce~East|cn~North "));
  comms->print(robot->imu.ypr.yaw/PI*180);
  comms->println(F("|cs~South|cm~Mow}"));
}
...
void RemoteControl::run(){  
  if (pfodState == PFOD_LOG_SENSORS){
      ...
      //comms->println("test");
      comms->print((float(millis())/1000.0f));
      comms->print(",");
      comms->print(robot->motorLeftSense);
      comms->print(",");
      comms->print(robot->motorRightSense);
      ...
      comms->print(robot->sonarDistRight);
      comms->print(",");
      comms->print(robot->perimeter.isInside(0));
      comms->print(",");
      comms->print(robot->perimeterMag);
      comms->print(",");
      comms->print(robot->odometryLeft);
      comms->print(",");
      comms->print(robot->odometryRight);
      comms->print(",");
      comms->print(robot->imu.ypr.yaw/PI*180);
      comms->print(",");
      comms->print(robot->imu.ypr.pitch/PI*180);
      ...
      comms->print(robot->imu.ypr.roll/PI*180);
      comms->print(",");
      comms->print(robot->imu.gyro.x/PI*180);
      comms->print(",");
      comms->print(robot->imu.gyro.y/PI*180);
      comms->print(",");
      comms->print(robot->imu.gyro.z/PI*180);
      comms->print(",");
      comms->print(robot->imu.acc.x);
      comms->print(",");
      comms->print(robot->imu.acc.y);
      comms->print(",");
      comms->print(robot->imu.acc.z);
      comms->print(",");
      comms->print(robot->imu.com.x);
      comms->print(",");
      comms->print(robot->imu.com.y);
      comms->print(",");
      comms->print(robot->imu.com.z);
      comms->print(",");
      float lat, lon;
      unsigned long age;
      robot->gps.f_get_position(&lat, &lon, &age);
      comms->print(robot->gps.hdop());
      comms->print(",");
      comms->print(robot->gps.satellites());
      comms->print(",");
      comms->print(robot->gps.f_speed_kmph());
      comms->print(",");
      comms->print(robot->gps.f_course());
      comms->print(",");
      comms->print(robot->gps.f_altitude());
      comms->print(",");
      comms->print(lat);
      comms->print(",");
      comms->print(lon);
    ...
    if (millis() >= nextPlotTime){
      ...
      comms->print(((unsigned long)millis()/60000));
      comms->print(",");
      comms->print(robot->batVoltage);
      comms->print(",");
      comms->print(robot->chgVoltage);
      ...
      comms->print(robot->chgCurrent);
      comms->print(",");
      comms->println(robot->batCapacity);
    }
  } else if (pfodState == PFOD_PLOT_ODO2D){
    if (millis() >= nextPlotTime){
      nextPlotTime = millis() + 500;
      comms->print(robot->odometryX);
      comms->print(",");
      comms->println(robot->odometryY);
    }
    ...
    if (millis() >= nextPlotTime){
      ...
      comms->print(robot->sonarDistCounter);
      comms->print(",");
      comms->print(robot->perimeterCounter);
      comms->print(",");
      comms->print(robot->lawnSensorCounter);
      comms->print(",");
      comms->print(robot->rainCounter);
      ...
      comms->print(robot->dropLeftCounter);
      comms->print(",");
      comms->println(robot->dropRightCounter);
    }
    ...
    if (millis() >= nextPlotTime){
      ...
      comms->print(robot->perimeter.isInside(0));
      comms->print(",");
      comms->print(robot->lawnSensor);
      comms->print(",");
      comms->print(robot->rain);
      comms->print(",");
      comms->print(robot->dropLeft);
      comms->print(",");
      comms->println(robot->dropRight);
    }
    ...
    if (millis() >= nextPlotTime){
      ...
      if (perimeterCaptureIdx < 32*3){      
        nextPlotTime = millis() + 200;            
        comms->print(perimeterCapture[perimeterCaptureIdx / 3]);
        comms->print(",");
        comms->print(robot->perimeterMag);
        comms->print(",");
        comms->print(robot->perimeter.getSmoothMagnitude(0));
        comms->print(",");
        comms->print(robot->perimeter.isInside(0));
        comms->print(",");
        comms->print(robot->perimeterCounter);
        comms->print(",");
        comms->print(!robot->perimeter.signalTimedOut(0));
        comms->print(",");
        comms->println(robot->perimeter.getFilterQuality(0));
        perimeterCaptureIdx++;
      }
    }
    ...
    if (millis() >= nextPlotTime){
      nextPlotTime = millis() + 500;
      comms->print(robot->gpsX);
      comms->print(",");
      comms->println(robot->gpsY);
    }
    ...
    if (millis() >= nextPlotTime){
      ...
      comms->print(robot->motorLeftRpmCurr);
      comms->print(",");
      comms->print(robot->motorRightRpmCurr);
      comms->print(",");
      comms->print(robot->motorLeftSpeedRpmSet);
      comms->print(",");
      comms->print(robot->motorRightSpeedRpmSet);
      comms->print(",");
      comms->print(robot->motorLeftPWMCurr);
      comms->print(",");
      comms->print(robot->motorRightPWMCurr);
      comms->print(",");
      comms->print(robot->motorLeftPID.eold);
      comms->print(",");
      comms->println(robot->motorRightPID.eold);
    }
  }
}
  ...
  while(comms->available() > 0){
    ...
    if (pfodCmdComplete) {
      ...
      if (pfodCmd == ".") sendMainMenu(false);      
          ...
          // log raw sensors
          comms->println(F("{=Log sensors}"));
          comms->print(F("time,leftsen,rightsen,mowsen,sonleft,soncenter,sonright,"));
          comms->print(F("perinside,permag,odoleft,odoright,yaw,pitch,roll,gyrox,gyroy,"));
          comms->print(F("gyroz,accx,accy,accz,comx,comy,comz,hdop,sats,gspeed,gcourse,"));
          comms->println(F("galt,lat,lon"));
          pfodState = PFOD_LOG_SENSORS;
        }  
        else if (pfodCmd == "y1") {
          // plot battery
          comms->println(F("{=battery|time min`0|battery V`1|charge V`1|charge A`2|capacity Ah`3}"));
          nextPlotTime = 0;
          pfodState = PFOD_PLOT_BAT;
        }
        else if (pfodCmd == "y2") {
          // plot odometry 2d
          comms->println(F("{=odometry2d|position`0~~~x|`~~~y}"));
          nextPlotTime = 0;
          pfodState = PFOD_PLOT_ODO2D;
        }
        else if (pfodCmd == "y3") {        
          // plot IMU
          comms->print(F("{=IMU`60|time s`0|yaw`1~180~-180|pitch`1|roll`1|gyroX`2~90~-90|gyroY`2|gyroZ`2|accX`3~2~-2|accY`3|accZ`3"));
          comms->println(F("|comX`4~2~-2|comY`4|comZ`4}"));
          nextPlotTime = 0;
          pfodState = PFOD_PLOT_IMU;
        }
        else if (pfodCmd == "y5") {        
          // plot sensor counters
          comms->print(F("{=Sensor counters`300|time s`0|state`1|motL`2|motR`3|motM`4|bumL`5|bumR`6"));
          comms->println(F("|son`7|peri`8|lawn`9|rain`10|dropL`11|dropR`12}"));
          nextPlotTime = 0;
          pfodState = PFOD_PLOT_SENSOR_COUNTERS;
        }
        else if (pfodCmd == "y6") {
          // plot perimeter spectrum
          /*comms->print(F("{=Perimeter spectrum`"));
          comms->print(Perimeter.getFilterBinCount());
          comms->print(F("|freq (Hz)`0|magnitude`0~60~-1|selected band`0~60~-1}"));*/
          comms->println(F("{=Perimeter`128|sig`1|mag`2|smag`3|in`4|cnt`5|on`6|qty`7}"));
          nextPlotTime = 0;
          pfodState = PFOD_PLOT_PERIMETER;          
        }
        else if (pfodCmd == "y7") {
          // plot sensor values
          comms->print(F("{=Sensors`300|time s`0|state`1|motL`2|motR`3|motM`4|sonL`5|sonC`6"));
          comms->println(F("|sonR`7|peri`8|lawn`9|rain`10|dropL`11|dropR`12}"));
          nextPlotTime = 0;
          pfodState = PFOD_PLOT_SENSORS;          
        }
        else if (pfodCmd == "y8") {
          // plot GPS 
          comms->println(F("{=GPS`300|time s`0|hdop`1|sat`2|spd`3|course`4|alt`5|lat`6|lon`7}"));
          nextPlotTime = 0;
          pfodState = PFOD_PLOT_GPS;          
        }
        else if (pfodCmd == "y10") {
          // plot GPS 2d
          comms->println(F("{=gps2d|position`0~~~x|`~~~y}"));
          nextPlotTime = 0;
          ...
          // motor control
          comms->println(F("{=Motor control`300|time s`0|lrpm_curr`1|rrpm_curr`2|lrpm_set`3|rrpm_set`4|lpwm`5|rpwm`6|lerr`7|rerr`8}"));
          nextPlotTime = 0;
